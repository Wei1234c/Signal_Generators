...
def calculate_regs(
    ...
    if not band_select_clock_divider:
        ...
        if band_select_clock_mode == BandSelectClockMode.Low:
            band_select_clock_divider = min(ceil(pfd_scale * pfd_freq), 255)
    ...
    return ref_freq, ref_doubler, r_counter, ref_div2, int(INT), int(MOD), int(
        FRAC), output_divider, band_select_clock_divider
